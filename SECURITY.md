...
ICOR for Life - Diagrams is a local-only Obsidian plugin. It makes no network requests and it
...
1. **GitHub private security advisory** (preferred). Go to the
   [Security tab](https://github.com/myICOR/icor-for-life-diagrams/security/advisories/new)
...
2. **Email** `support@myicor.com` with `SECURITY` and `icor-for-life-diagrams` in the subject
...
- Steps to reproduce, ideally against a throwaway vault, with the diagram source
...
ICOR for Life - Diagrams is a local rendering surface. Measured against the shipped `main.js`
of v0.1.0:
...
- **No network access.** The bundle contains zero `fetch`, `requestUrl` and
  ...
  not phone home, and there is no telemetry or analytics. Nothing is fetched from a
  CDN at render time.
- **No credentials.** The plugin stores no API keys, tokens or passwords. It has
  no account, no login, and nothing to leak.
- **Local scope only.** It reads diagram source from your notes, renders it, and
  stores its own settings through Obsidian's plugin settings API.
...
- **Injection through diagram source.** This is the sharpest edge here: the plugin
  turns text from a note into rendered output. A crafted diagram block that results
  in script execution, HTML injection into the pane, arbitrary file read or write,
  or process spawning is exactly the report we want.
- Rendering a diagram that causes the plugin to read a file outside the vault, or
  outside the folders it is configured to use.
- SVG or markup output that escapes its container and executes in the Obsidian
  renderer.
- A diagram source string that causes unbounded resource consumption sufficient to
  hang or crash Obsidian, where the source could plausibly arrive from a shared or
  synced note.
...
- Diagrams that render incorrectly, or layout and styling bugs. Those are ordinary
  issues and very welcome as such, just not through this channel.
